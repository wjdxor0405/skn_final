...
납품 가능성 검증도 Odoo 없이 돕니다 — `39-01-2040` / 30일로 수량을 올려보면:
...
`/api/feasibility/procure` 로 부족분 조달까지 이어지면 협상이 실행됩니다.
...
- `snapshot` 모드는 발주서를 만들지 않습니다 — 외부 ERP가 없습니다
- `snapshot` 모드의 납품 가능성 검증은 BOM 전개를 건너뜁니다 — Octopart에는 BOM이 없고 '우리' 재고도 없어서, 모든 품목을 재고 0인 구매 품목으로 봅니다. 조달 납기·단가·공급 한도는 실제 유통사 데이터에서 옵니다
