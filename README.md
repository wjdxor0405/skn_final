...
**남은 구멍 하나 — `llm` 모드의 자유 문장.** 바이어 에이전트는 수락/거절을 판단하려면
상한가를 알아야 하므로 프롬프트에는 액수가 들어갑니다(`llm_agents.py`). 그 LLM이 상대에게
보낼 메시지에 액수를 그대로 적어버릴 수 있는데, **프롬프트에 "금액을 밝히지 말라"는 지시가
없고** `audit.py`는 가격 판단만 검사하고 메시지 본문은 검사하지 않습니다.
`rule` 모드(기본값)에는 이 구멍이 없습니다.
...
- `llm` 모드에서 바이어 에이전트의 자유 문장에 예산 액수가 새어나갈 수 있음 — 프롬프트 지시도 출력 검사도 없음 (위 "정보 은닉" 절 참고). `rule` 모드는 해당 없음
## 라이선스와 오픈소스 사용 고지
